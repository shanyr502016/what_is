"""
Action: ACL Import
Command: am_install_tree -u= -g= -pf= -mode= -format= -path=            
"""
import os
...
class ACL(Loggable):
        ...
        for index, server_info in enumerate(_execute_servers):
            ...
            for tc_package_id in self.__environment_provider.getTCPackageID(self._tcpackage_id):
                ...
                try:
                    if os.path.exists(_location_in_package):
                        for file in os.listdir(_location_in_package):
                            if file.endswith('.xml'):
                                ...
                                self.log.info(f"ACL Import Command: {command}")
                                result = self._execute(command, _location_in_package)
                                self._processes.append({'NODE':server_info['NODE'],'process': result,'module': self.__module_label, 'package_id': tc_package_id,'label': f"[{tc_package_id}] - ACL Import "})
                                if not self._parallel:
                                    self.__console_msg(result,  f"[{tc_package_id}] - ACL Import")
                            else:
                    ...
                    self.log.error(exp)
                    self._processes.append({'NODE':server_info['NODE'],'process': 1,'module': self.__module_label, 'package_id': tc_package_id,'label': f"[{tc_package_id}] - ACL Import "})
    ...
    def __build_arguments(self, server_info, file):
        ...
        if self.__environment_provider.get_property_validation('mode', self.__properties[self.__target]):
            args.append('-mode='+ self.__properties[self.__target]['mode'])

        if self.__environment_provider.get_property_validation('format', self.__properties[self.__target]):
            args.append('-format='+self.__properties[self.__target]['format'])
