...
class AIG(Loggable):
                    ...
                    for retry_count in range(_retry): 
                        ...
                        if self.__remote_execution and self.__is_linux:
                            ...
                            if self.__operation == 'start':
                                command.append(f'"cd {_location} && ./restart"')
                            else:
                                command.append(f'"cd {_location} && ./{self.__operation}"')
                            working_dir.append('/')
                        elif not self.__remote_execution and self.__is_linux:
                            if self.__operation == 'start':
                                command.append(os.path.join(_location, 'restart'))
                            else:
                                command.append(os.path.join(_location, self.__operation))
                            working_dir.append(_location)
                            
                        if self.__remote_execution and self.__is_windows:
                            command.append(os.path.join(_location, self.__operation))
                            working_dir.append(_location)
                        elif not self.__remote_execution and self.__is_windows: 
                            command.append(os.path.join(_location, self.__operation))
                            working_dir.append(_location)
                        ...
                        # Get Service Status with returns print services informations
                            result = self._execute(command, working_dir, __ssh_command, _service_name)
                            ...
                            self.log.info(f"Requesting the {__class__.__name__} {_service_name} {self.__action} from {instance['NODE']} [{nodes['HOSTNAME']}]")
                            result = self._execute(command, working_dir,  __ssh_command, _service_name)
    ...
    def _execute(self, command, path, ssh_command, _service_name):
