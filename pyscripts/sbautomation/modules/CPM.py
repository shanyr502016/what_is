...
class CPM(Loggable):
    ...
    def _build_arguments(self):
        ...
        for inscount, instance in enumerate(self.__instances):
            ...
            # CPM LOCATION and Coomand
            _cpm_location = instance['CPM_LOCATION']
            _service_name = instance['SERVICE_NAME']
            self.__cpm_location = _cpm_location
            ...
            if _instance_count > 0:
                ...
                for nodecount, nodes in enumerate(_execute_servers): 
                    ...
                    for retry_count in range(_retry):
                        ...
                        command.append(_cpm_location + self.__operation)
                        ...
                        if self.__print_services: 
                            ...
                            self.log.info(f"Requesting to {self.__action} the {__class__.__name__} from {instance['NODE']} [{nodes['HOSTNAME']}]")
                            result = self._execute(command, working_dir, __ssh_command)
    ...
    def _execute(self, command, path, ssh_command):
        ...
        if self.__is_linux:
            ...
            process.set_servicename('cpm') 
            process.set_action(self.__action)
            process.set_command_regex(self.__cpm_location)
