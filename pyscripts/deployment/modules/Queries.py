...
class Queries(Loggable):
    ...
    def default(self):
        ...
        self.log.info("Queries Import All")
    ...
    def customquery(self):           
        ...
        """
        Action: customquries
        Command:  plmxml_import -u= -g= -pf= import_mode= -xml_file=
        """      
        for index, server_info in enumerate(_execute_servers):            
            self.log.info(f"Queries Import")
            for tc_package_id in self.__environment_provider.getTCPackageID(self._tcpackage_id):
                try:
                    ...
                    if os.path.exists(_location_in_package):
                        for file in os.listdir(_location_in_package):
                            if file.endswith('.xml'):
                                ...
                                self.log.info(f"Queries_Import Command: {command}") 
                        ...
                        if __commands:
                            utilityExecution = UtilityExecutionSet(self.__environment_provider, __commands, 'Queries_' + tc_package_id, self._parallel)
                            result = utilityExecution.execute()
                            self._processes.append({'NODE':server_info['NODE'],'process': result, 'module': self.__module_label, 'package_id': tc_package_id, 'label': f"[{tc_package_id}] - Queries Import"})            
                            if not self._parallel:
                                self.__console_msg(result, f"[{tc_package_id}] - Queries Import")
                        else:
                    ...
                    self.log.error(exp)
                    self._processes.append({'NODE':server_info['NODE'],'process': 1, 'module': self.__module_label, 'package_id': tc_package_id, 'label': f"Queries Import"})   
            ...
            for threadcount, targets in enumerate(self.__environment_provider.get_execute_targets(self.__module_name).split(',')): 
                ...
                if not self.__sub_module_name:   
                    ...
                    if dynamicExecutor.get_sub_module_name():
                        _processesResult = getattr(Queries, dynamicExecutor.get_sub_module_name())(self)   
                    else:
