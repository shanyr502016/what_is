...
class Pool(Loggable):
    ...
    def _build_arguments(self):
        ...
        for inscount, instance in enumerate(self.__instances):
            ...
            # POOL Location and RC filepath
            _location = instance['POOL_LOCATION']
                    ...
                    for retry_count in range(_retry):
                        ...
                        if self.__print_services:
                            ...
                            if self.__operation == 'stop':
                                self.run_deployment('Clearlocks')

                        if self.__operation == 'stop':
                            self.depends_service(instance, __ssh_command)
    ...
    def run_deployment(self, module):
        """
        Executing the target from Deployment modules
        """
        self.log.info(f"Requesting to execute the {module}")
        dynamicExecutor = DynamicExecutor(self.__arguments)
        dynamicExecutor.run_deploy(Constants.PACKAGEINFO_DEPLOYMENT, module)
    ...
    def _execute(self, command, path, ssh_command):
        ...
        if self.__is_linux:
            ...
            process.set_servicename('poolmanager.ServerManager')
    ...
    def depends_service(self, services, ssh_command):
        """ 
        Depends Services
        """
        if "DEPENDS_SERVICE_NAMES" in services:
            process = Process('',ssh_command = ssh_command)
            for service_name in services["DEPENDS_SERVICE_NAMES"].split(','):
                self.log.info(f'Requesting to Kill the Depends Services {service_name}')
                process.set_servicename(service_name)
                pid = process.get_process_id()
                if pid:
                    result = process.kill_service(pid)
                else:
                    result = pid
                if not result:
                    
                    self.log.info(Constants.colorize(f'Depends Services {service_name} has Stopped Successfully\n', Constants.TEXT_LIGHT_GREEN))
                else:
                    self.log.info(Constants.colorize(f'Depends Services {service_name} has Stopped Successfully\n', Constants.TEXT_LIGHT_RED))
